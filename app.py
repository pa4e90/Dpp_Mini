    ...
    try:
        ...
        if missing:
            ...
            st.session_state.data = (
                pd.concat([st.session_state.data, ensure_columns(up)], ignore_index=True)
            ...
            if dropped_gtin or dropped_exp:
                parts = []
                if dropped_gtin: parts.append(f"invalid GTIN: {dropped_gtin}")
                if dropped_exp:  parts.append(f"past expiry: {dropped_exp}")
...
if st.button("Validate & Add"):
    try:
        ...
        if st.session_state.cfg["enforce_future_expiry"] and not expiry_is_future_or_today(item.expiry.isoformat()):
            st.error("Expiry is in the past and 'Disallow past expiry dates' is ON.")
        else:
            st.session_state.data.loc[len(st.session_state.data)] = [
                item.gtin, item.batch, item.expiry.isoformat(), now_iso()
            ]
...
if enable_delete and not df_sorted.empty:
    for i, row in df_sorted.iterrows():
        cols = st.columns([5, 2])
        with cols[0]:
            st.write(f"{row['gtin']}  |  {row['batch']}  |  {row['expiry']}  |  {row['created_at']}")
        with cols[1]:
